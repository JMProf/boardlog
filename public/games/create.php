...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') { $err = 'El nombre es obligatorio.'; }
        ...
        try {
            $id = GameModel::create($name, current_user()['id']);
            AuditModel::log('games', 'create', $id, ['name'=>$name], current_user()['id']);
            header('Location: /games/list.php'); exit;
        } catch (Throwable $e) { $err = 'No se pudo crear. ¿Existe ya un juego con este nombre?'; }
    }
}
