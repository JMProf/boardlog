...
if (!$play) { http_response_code(404); exit('No encontrado'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (($e = validate_date_past_or_present($date))) { $err = $e; }
        ...
        PlayModel::update($id, $game_id, $players, $date, current_user()['id']);
        AuditModel::log('plays', 'update', $id, ['game_id'=>$game_id,'players_count'=>$players,'played_at'=>$date], current_user()['id']);
    ...
    <div class="mb-3">
      ...
      <select class="form-select" name="game_id" required>
        <?php foreach ($games as $g): ?>
          <option value="<?= (int)$g['id'] ?>" <?= (int)$g['id']===(int)$play['game_id']?'selected':'' ?>><?= htmlspecialchars($g['name']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="mb-3">
      <label class="form-label">Número de jugadores</label>
      <input class="form-control" type="number" name="players_count" min="1" max="100" value="<?= (int)$play['players_count'] ?>" required>
    </div>
    <div class="mb-3">
      <label class="form-label">Fecha</label>
      <input class="form-control" type="date" name="played_at" value="<?= htmlspecialchars($play['played_at']) ?>" required>
    </div>
